...
- `kafka-detector-forwarder`: Kafka consumer that sends body-event records to detector Envoy
...
- traffic Envoy ingress to `credit-ui`: http://localhost:8088
...
local `kafka-detector-forwarder` module is the Kafka consumer that sends topic
...
To capture body events for user traffic and inter-service calls:
...
3. Open the UI through the traffic Envoy:
...
- Browser -> traffic Envoy -> `credit-ui`
- `credit-ui` -> traffic Envoy -> `credit-input`
...
Kafka through Confluent Kafka REST Proxy. The local `kafka-detector-forwarder`
...
```text
traffic Envoy -> Kafka REST Proxy -> Kafka topic -> kafka-detector-forwarder -> detector Envoy -> logs/envoy-pii-detection.log -> Alloy
```
...
This builds all Java modules, including `kafka-detector-forwarder`.
...
consumer. In this setup, the `kafka-detector-forwarder` module is that consumer.
It uses Apache Kafka's Java client, consumes `envoy-body-events`, and POSTs each
record value to `http://[::1]:8091/detect` with `Content-Type: application/json`.
...
Open the UI through the traffic Envoy and submit an application:
...
curl -i -sS \
  ...
  http://localhost:8088/api/credit/apply
```
...
- Traffic Envoy publishes request/response body events directly to Kafka through Kafka REST Proxy.
- Detector Envoy inspects Kafka-fed payloads in memory, converts PII-like strings into numeric counters, and writes only derived counts to `logs/envoy-pii-detection.log`.
...
```text
traffic Envoy body event -> Kafka -> kafka-detector-forwarder -> detector Envoy Lua scan -> derived count fields -> Alloy metrics
```
...
Alloy exposes counters such as:
